...
export function useMoments() {
  ...
  useEffect(() => {
    ...
    const fetchMoments = async () => {
      try {
        const request = axiosInstance.get<Moment[]>("/moments", {
  ...
  const addMoment = async (moment: CreateMoment) => {
    ...
    try {
      ...
      const response = await axiosInstance.post<Moment>("/moments", formData, {
  ...
  const deleteMoment = async (moment: Moment) => {
    try {
      ...
      await axiosInstance.delete("/moments/" + moment.id);
    } catch (err) {
      setError((err as AxiosError).message);
  ...
  const updateMoment = async (moment: Moment) => {
    ...
    try {
      ...
      await axiosInstance.put("/moments/" + moment.id, moment);
    } catch (err) {
      setError((err as AxiosError).message);
      setMoments((curr) =>
